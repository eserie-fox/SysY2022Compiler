...
TEST(TACFactory, AccessArray) {
  ...
  auto ret = builder->AccessArray(arraySym, {1, 1});
  ...
  for (size_t i = 0; i < 2; i++) {
    for (size_t j = 0; j < 2; j++) {
      for (size_t k = 0; k < 3; k++) {
        auto array = builder->AccessArray(arraySym, {i, j, k});
  ...
  for (size_t i = 0; i < 2; i++) {
    for (size_t j = 0; j < 2; j++) {
      auto array = builder->AccessArray(arraySym, {i, j});
...
TEST(TACFactory, ArrayInit1) {
  ...
  { array2->subarray->emplace(0, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(0))); }
  { array2->subarray->emplace(1, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(1))); }
  { array2->subarray->emplace(2, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(2))); }
  { array2->subarray->emplace(3, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(3))); }
  builder->MakeArrayInit(arraySym, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(array2)));
...
TEST(TACFactory, ArrayInit3) {
  ...
  auto array2 = builder->NewArrayDescriptor();
  {
    ...
    array3->subarray->emplace(0, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(0)));
    array3->subarray->emplace(1, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(1)));
    array2->subarray->emplace(0, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(array3)));
  }
  { array2->subarray->emplace(2, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(2))); }
  { array2->subarray->emplace(3, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(3))); }
  builder->MakeArrayInit(arraySym, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(array2)));
...
TEST(TACFactory, ArrayInit4) {
  ...
  { array2->subarray->emplace(1, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(2))); }
  { array2->subarray->emplace(2, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(3))); }
  builder->MakeArrayInit(arraySym, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(array2)));
  ASSERT_EQ(2, array1->subarray->size());
...
TEST(TACFactory, ArrayInit5) {
  ...
  {
    auto array3 = builder->NewArrayDescriptor();
    array3->subarray->emplace(0, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(2)));
    array2->subarray->emplace(1, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(2)));
  }
  builder->MakeArrayInit(arraySym, builder->NewSymbol(SymbolType::Constant, std::nullopt, 0, SymbolValue(array2)));
  ASSERT_EQ(2, array1->subarray->size());
