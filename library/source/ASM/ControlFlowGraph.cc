...
ControlFlowGraph::ControlFlowGraph(TACListPtr FuncTACList)
    : ControlFlowGraph(FuncTACList->begin(), FuncTACList->end()) {}
...
ControlFlowGraph::ControlFlowGraph(TACList::iterator fbegin, TACList::iterator fend) {
  ...
  // 合法性检查
  if (fbegin == fend) throw std::runtime_error("ControlFlowGraph error: empty tac list");
  --fend;
  ...
  for (auto e : jmpReloc) link(e, labelMap[getJmpLabel(nodes[e].tac)]);

  setDfn();

  // 检查不可达代码并将其结点从流图中删除(bug: 不删导致活跃分析访问到不可达前驱)
  for (size_t i = 0; i < nodes.size(); ++i) {
    if ((*itrMap[i])->operation_ == TACOperationType::FunctionBegin ||
        (*itrMap[i])->operation_ == TACOperationType::FunctionEnd) {
      // fbegin和fend不许删除
      continue;
    }
    if (nodes[i].dfn == 0) {
...
inline std::string ControlFlowGraph::getJmpLabel(TACPtr tac) { return tac->a_->get_name(); }
...
void ControlFlowGraph::WarnUnreachable() const {
  if (!unreachableTACItrList.empty()) {
    std::cerr << "WARNING: function \"" << (*f_begin)->a_->get_name() << "\" contains unreachable statements:\n";
    for (auto itr : unreachableTACItrList) std::cerr << (*itr)->ToString() << '\n';
    std::cerr << '\n';
  }
}
